...
# test identity
{
    ...
    ok( $s->is_identical( $class->new( [@supplier_tuples] ) ),
        'relation is === to relation with same members'
    );

    isa_ok( $p, $class );
    ok( $p->is_identical($p), 'relation is === to itself' );
    ok( $p->is_identical( $class->new( [@part_tuples] ) ),
        'relation is === to relation with same members'
    );

    isa_ok( $sp, $class );
    ok( $sp->is_identical($sp), 'relation is === to itself' );
    ok( $sp->is_identical( $class->new( [@shipment_tuples] ) ),
        'relation is === to relation with same members'
    );
    ...
    ok( !$s->is_identical(
            $class->new( [ $supplier_tuples[0] ] ) ),
        'relations of same class but different members are not ==='
    );
    ok( !$s->is_identical( $class->new( [] ) ),
        'relations of same class but different members are not ==='
    );
}
...
# restriction
{
    ...
    my $expect = $class->new( [ $supplier_tuples[0] ] );
...
{
    diag('union');
    my $s1 = $class->new( [ @supplier_tuples[ 0, 1, 2 ] ] );
    my $s2 = $class->new( [ @supplier_tuples[ 1, 2, 3, 4 ] ] );
    my $s3 = $s1->union($s2);
...
# insertion
{
    ...
    my $expect = $s->union( $class->new( [$inserted] ) );
...
# intersection
{
    ...
    my $s1 = $s->intersection(
        $class->new( [ @supplier_tuples[ 0, 4 ], $another ] )
    );
    my $expect = $class->new( [ @supplier_tuples[ 0, 4 ] ] );
